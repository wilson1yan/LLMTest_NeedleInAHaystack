...
tokens_cache = None
question_to_ask = "What are the top ten most fun things to do in San Francisco?"
needles = [
    "The first most fun thing to do in San Francisco is to explore the Sutro Baths Ruins.",
    "The second most fun thing to do in San Francisco is to visit the Wave Organ, a unique sound sculpture",
    "The third most fun thing to do in San Francisco is to tour the Musée Mécanique",
    "The fourth most fun thing to do in San Francisco is to stroll through the Secret Tiled Staircase",
    "The fifth most fun thing to do in San Francisco is to attend a show at the historic Castro Theatre",
    "The sixth most fun thing to do in San Francisco is to explore the Camera Obscura at Ocean Beach",
    "The seventh most fun thing to do in San Francisco is to visit the fortune cookie factory in Chinatown",
    "The eighth most fun thing to do in San Francisco is to walk through the Presidio Pet Cemetery",
    "The ninth most fun thing to do in San Francisco is to see the wild parrots on Telegraph Hill",
    "The tenth most fun thing to do in San Francisco is to check out the quirky street art at Clarion Alley"
]

question_to_ask = "Who starts with the ball, and what is the sequence of people it is passed to?"
needles = [
    "Alice starts with a ball",
    "Alice gives the ball to John",
    "John gives the ball to George",
    "George gives the ball to Bob",
    "Bob gives the ball to Mark"
]
...
def generate_context(needles, context_length, llama_tokenizer):
    # Load up tiktoken so we navigate tokens more easily
    np.random.seed(context_length)
    ...
    for needle in needles:
        depth_percent = np.random.uniform(0, 1)
...
class Sampler:
    ...
    def __call__(self, context):
        ...
        if 'vicuna' in self.ckpt_path or 'longchat' in self.ckpt_path:
            ...
            prompt = f"{system_prompt} USER: {context} Given the context above, answer the following question. {question_to_ask}  Assistant: "
...
def eval_example(inp, model_to_test):
    ...
    context = generate_context(needles, context_length, model_to_test.tokenizer)
